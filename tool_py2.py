...
class Optimizer:
    def __init__(self):
        self.default_args = '-XX:+PrintGCDetails -XX:+UnlockDiagnosticVMOptions \
                             ...
                             -XX:+PrintHeapAtGC -XX:+PrintHeapAtGCExtended -XX:+PrintGCTimeStamps \
    ...
    def warm_up(self):
        ...
        cmd = 'java -Xmx{0} -Xms{0} -jar {1} {2} >/dev/null 2>&1'.format(heap_size, jar_path, run_argument)
    ...
    def search_arguments(self, opt_arg):
        ...
        while (index[0] < arguments[0][1][1]):
            ...
            cmd += cmd_arg
            cmd += '-jar {0} {1} >/dev/null 2>&1'.format(jar_path, run_argument)
            st = time.time()
            os.system(cmd)
            et = time.time()
    ...
    def optimize(self):
        ...
        cmd = 'java ' + self.default_args + append_args + '-jar {0} {1} >/dev/null 2>&1'.format(jar_path, run_argument)
    ...
    def execution_time(self):
        ...
        min_index = -1
        for i in test_range:
            cmd = 'java -Xmx{0} -Xms{0} -XX:{1}={2} -jar {3} {4} >/dev/null 2>&1'.format(heap_size, test_argument, i, jar_path, run_argument)
            sum = 0
            for j in range(0, 3):
                st = time.time()
                os.system(cmd)
